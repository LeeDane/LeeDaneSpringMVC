...
public class ZanServiceImpl implements ZanService<ZanBean>{
	...
	public Map<String, Object> addZan(JSONObject jo, UserBean user,
		...
		if(zanMapper.exists(tableName, tableId, user.getId())){
		...
		if(!zanMapper.recordExists(tableName, tableId)){
		...
		if(result){
			...
			createUserId = zanMapper.getObjectCreateUserId(tableName, tableId);
