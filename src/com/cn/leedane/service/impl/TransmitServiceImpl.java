...
public class TransmitServiceImpl implements TransmitService<TransmitBean>{
	...
	public Map<String, Object> add(JSONObject jo, UserBean user,
		...
		if(transmitMapper.save(bean) > 0){
			...
			createUserId = transmitMapper.getObjectCreateUserId(tableName, tableId);
	...
	public int getTotalTransmits(int userId) {
		return this.transmitMapper.getTotalTransmits(userId);
	}
	...
	public Map<String, Object> updateTransmitStatus(JSONObject jo, UserBean user,
		...
		boolean result = transmitMapper.updateSQL("update "+ tableName + " set can_transmit=? where id=?", canTransmit, tableId);
