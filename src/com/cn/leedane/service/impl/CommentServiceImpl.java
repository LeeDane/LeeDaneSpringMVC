...
public class CommentServiceImpl implements CommentService<CommentBean>{
	...
	public Map<String, Object> add(JSONObject jo, UserBean user,
		...
		if(commentMapper.save(bean) > 0){
			...
			if(pid > 0){//说明是回复别人的评论
				...
				createUserId = commentMapper.getObjectCreateUserId(tableName, tableId);
	...
	public int getCountByObject(JSONObject jo, UserBean user,
		...
		int count = 0;
		count = commentMapper.getTotal(DataTableType.评论.value, sql.toString());
		//保存操作日志
	...
	public int getTotalComments(int userId) {
		return this.commentMapper.getTotalComments(userId);
	}
	...
	public Map<String, Object> updateCommentStatus(JSONObject jo, UserBean user,
		...
		boolean result = commentMapper.updateSQL("update "+ tableName + " set can_comment=? where id=?", canComment, tableId);
